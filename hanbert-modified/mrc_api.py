...
## Required parameters
flags.DEFINE_string(
    "bert_config_file", "/data/data/bert/hanbert/HanBert-54kN/HanBert-54kN/bert_config.json",
    "The config json file corresponding to the pre-trained BERT model. "
    "This specifies the model architecture.")

flags.DEFINE_string("vocab_file", "/data/data/bert/hanbert/HanBert-54kN/HanBert-54kN/vocab_54k.txt",
                    "The vocabulary file that the BERT model was trained on.")

## Other parameters
## independent of codalab submission, used for init_checkpoint on prediction
flags.DEFINE_string("example_file", "/data/data/bert/KorQuAD_v1.0_dev.json",
                    "The vocabulary file that the BERT model was trained on.")

flags.DEFINE_string(
    "init_checkpoint", '/data/data/bert/hanbert/HanBert-54kN/54kN-result',
...
flags.DEFINE_string(
    "input_file_dir", '/data/data/bert/data',
    "input file dir"
    "input file dir")
...
def read_text_examples(input_context, input_question):
  """Read a SQuAD json file into a list of SquadExample."""
  #input_context = input_context[0]
  #print('read_text_examples input_context = ', input_context)
  #print('read_text_examples input_question = ', input_question)
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
  ...
  for (example_index, example) in enumerate(examples):
    ...
    print('################ token len = ', len(example.doc_tokens))
...
def input_fn_builder(input_context, input_question, tokenizer, seq_length, drop_remainder):
    """Creates an `input_fn` closure to be passed to TPUEstimator."""

    def gen():
        print(f"본문내용 : {input_context}\n\n")
        #print('input_fn_builder input_context =', input_context)
        #print('input_fn_builder input_question =', input_question)

        examples = read_text_examples(input_context, input_question)
        ...
        a = {'unique_ids': [f.unique_id for f in features],
        ...
        print(a)
        '''
        print('input_ids size =', len(a['input_ids']))
        for i, input_id in enumerate(a['input_ids']):
            print('input_id =', i, input_id)
        print('input_mask size =', len(a['input_mask']))
        for i, input_mask in enumerate(a['input_mask']):
            print('input_mask =', i, input_mask)
        print('input_mask size =', len(a['input_mask']))
        for i, segment_id in enumerate(a['segment_ids']):
            print('segment_ids =', i, segment_id)
        '''
        yield a
...
def write_predictions(all_examples, all_features, all_results, n_best_size,
  ...
  for (example_index, example) in enumerate(all_examples):
    ...
    for i, entry in enumerate(nbest_json):
        print('nbest = ', entry)
...
def read_korquad_examples():
  ...
  flist = os.listdir(FLAGS.input_file_dir)
  examples = []
  for entryf in flist:
    input_data = ''
    with tf.gfile.Open(FLAGS.input_file_dir+'/'+ entryf, "r") as f:
        lines = f.readlines()
        for line in lines:
            input_data += line
